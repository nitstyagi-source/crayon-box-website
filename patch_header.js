        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
             ...
             <div className="flex items-center gap-1.5">
               <h1 className="text-[20px] text-slate-900 font-extrabold leading-tight">{currentRole.replace('_', ' ')}</h1>
               <div className="w-4 h-4 rounded-full bg-blue-500 text-white flex items-center justify-center">
                 <Check className="w-3 h-3" strokeWidth={3} />
               </div>
             </div>
             <p className="text-slate-500 text-[13px] font-medium mt-0.5">{selectedInstitutionObj?.name || 'All Institutions (Trust HQ)'}</p>
          </div>
        </div>

        {/* Right: Actions */}
        <div className="flex items-center gap-4">
           {/* Global Command Search (from old shell) */}
           <button onClick={onOpenSearch} className="relative w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-indigo-600 hover:border-indigo-200 transition shadow-xs group">
