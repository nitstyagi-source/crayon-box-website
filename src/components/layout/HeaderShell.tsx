        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5 sm:gap-3 w-full max-w-7xl mx-auto">
          ...
          <div className="relative flex-1 sm:max-w-[280px]">
            <button
              ...
              className="w-full flex items-center justify-between px-2 py-1.5 rounded-lg border border-slate-200 bg-slate-50 hover:bg-white transition text-[11px] font-bold text-slate-800 shadow-2xs cursor-pointer"
            >
              <div className="flex items-center gap-2 truncate">
                <SchoolLogo 
                  code={currentInstitution} 
                  logoUrl={isAllInstitutions ? '/trust-logo.png' : activeLogo}
                  size="xs" 
                  shape="square" 
            ...
            {isInstOpen && (
              <div className="absolute top-full left-0 mt-1.5 w-full min-w-[280px] bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 z-50 animate-in fade-in">
                <button
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <span className="block leading-tight font-black">🏛️ Trust HQ</span>
                      <span className="text-[9px] text-slate-400 block font-normal leading-tight">All Campuses Consolidated</span>
                    </div>
                  </div>
                ...
                {institutionsList.map((inst) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 truncate">
                      <SchoolLogo 
                        code={inst.code} 
                        logoUrl={inst.logoUrl || inst.logo_url} 
                        name={inst.name}
                        size="sm" 
                        shape="square" 
                      />
                      <div className="truncate">
                        <span className="truncate block leading-tight">{inst.name}</span>
                        <span className="text-[9px] text-slate-400 font-mono block leading-tight">{inst.code} • {inst.boardAffiliation || 'Recognized'}</span>
                      </div>
                    </div>
                    {currentInstitution === inst.code && <Check className="w-4 h-4 text-indigo-600 shrink-0 ml-2" />}
                  </button>
                ))}
              </div>
            )}
