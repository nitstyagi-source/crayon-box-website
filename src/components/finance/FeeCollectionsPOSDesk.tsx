            ...
            <div className="bg-emerald-50/50 border border-emerald-200 p-6 rounded-3xl space-y-4 animate-in fade-in">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[10.5px] bg-stone-50/70 p-3 rounded-xl border border-stone-100">
                  <div>
                    <span className="text-stone-400">Receipt No:</span> <strong className="text-stone-900 font-mono">{generatedReceipt.receipt_no}</strong>
                  </div>
                  <div>
                    <span className="text-stone-400">Date:</span> <strong className="text-stone-900 font-mono">{generatedReceipt.receipt_date}</strong>
                  </div>
                  <div>
                    <span className="text-stone-400">Student Name:</span> <strong className="text-stone-900">{generatedReceipt.student_name}</strong>
                  </div>
                  <div>
                    <span className="text-stone-400">Admission No:</span> <strong className="text-stone-900 font-mono">{generatedReceipt.admission_no}</strong>
                  </div>
                  <div>
                    <span className="text-stone-400">Class & Section:</span> <strong className="text-stone-900">{generatedReceipt.class_name} {generatedReceipt.section_name}</strong>
                  </div>
                  <div>
                    <span className="text-stone-400">Parent / Guardian:</span> <strong className="text-stone-900">{generatedReceipt.parent_name}</strong>
                  </div>
                </div>
                ...
                <div className="border-t border-b border-stone-200 py-2.5 space-y-1.5 text-[11px]">
                  <div className="flex justify-between">
                    ...
                    <strong className="text-stone-900">{generatedReceipt.payment_mode} ({generatedReceipt.transaction_ref || 'Counter POS'})</strong>
                  </div>
                  <div className="flex justify-between text-stone-600">
                    <span>Total Session Demand:</span>
                    <span>{formatCurrency(generatedReceipt.total_amount_due)}</span>
                  </div>
                  {Number(generatedReceipt.concession_amount) > 0 && (
                    <div className="flex justify-between text-purple-700">
                      <span>Concession Applied:</span>
                      <span>- {formatCurrency(generatedReceipt.concession_amount)}</span>
                    </div>
                  )}
                  {Number(generatedReceipt.late_fee_amount) > 0 && (
                    <div className="flex justify-between text-red-700">
                      <span>Late Fee Penalty:</span>
                      <span>+ {formatCurrency(generatedReceipt.late_fee_amount)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm font-black text-stone-900 pt-1.5 border-t border-dashed border-stone-200">
                    <span>Total Amount Paid:</span>
                    <span className="text-emerald-700 font-mono">{formatCurrency(generatedReceipt.net_amount_paid)}</span>
                  </div>
                  <div className="flex justify-between text-stone-500 text-[10.5px]">
                    <span>Remaining Balance Due:</span>
                    <span className="font-bold text-amber-600 font-mono">{formatCurrency(generatedReceipt.remaining_balance)}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-end pt-1 text-[9.5px] text-stone-500">
                  <div className="space-y-0.5">
                    <p>Cashier / In-Charge: <strong className="text-stone-800">{generatedReceipt.collected_by || 'Accounts Desk'}</strong></p>
