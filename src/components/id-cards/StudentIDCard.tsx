...
import { IdCardCustomConfig, DEFAULT_ID_CARD_CONFIG, getIdCardConfig } from '@/lib/id-card-config';
...
export interface StudentIDCardProps {
  ...
  config?: Partial<IdCardCustomConfig>;
}
 ...
 * Exact 1:1 visual match to user reference image with fully customizable branding, colors, fields, and mottos.
 */
  ...
  if (layoutMode === 'DUAL' || layoutMode === 'both') {
    return (
      <div className="flex flex-wrap items-start justify-center gap-6">
        <StudentIDCard student={student} schoolInfo={schoolInfo} isBack={false} layoutMode="FRONT_ONLY" config={propsConfig} />
        <StudentIDCard student={student} schoolInfo={schoolInfo} isBack={true} layoutMode="BACK_ONLY" config={propsConfig} />
      </div>
    );
  }

  const institutionCode = schoolInfo?.code || s?.institution_code || s?.school_code || 'DEFAULT';
  const [persistedConfig, setPersistedConfig] = useState<IdCardCustomConfig>(() => getIdCardConfig(institutionCode));

  useEffect(() => {
    setPersistedConfig(getIdCardConfig(institutionCode));
    ...
    window.addEventListener('idcard_config_updated', handleConfigUpdate);
    return () => {
      window.removeEventListener('idcard_config_updated', handleConfigUpdate);
    };
  }, [institutionCode]);

  // Combine defaults, persisted config, and props override
  const cfg: IdCardCustomConfig = useMemo(() => {
    return {
    ...
    return (
      ...
      >
        ...
        <div className="text-center pt-2 pb-1 z-10 shrink-0">
          ...
          >
            {cfg.backHeaderTitle || 'STUDENT INFORMATION'}
          </h2>
          <div 
          ...
          <div style={{ color: cfg.primaryColor }} className="space-y-1.5 text-[10.5px] leading-tight">
            ...
            <div className="flex items-start gap-2">
              ...
              <span className="font-medium">
                {cfg.guideline2.includes('{schPhone}') 
                  ? cfg.guideline2.replace('{schPhone}', schPhone)
                  : `${cfg.guideline2.replace(/\.?$/, '')} (${schPhone}).`}
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span 
      ...
      <div className="px-5 pt-1 pb-0 flex-1 flex flex-col justify-around z-10">
        <div className="flex items-center justify-between">
          ...
          <div className="flex-1 pl-4 flex flex-col justify-center">
            ...
            <h2 
              style={{ color: cfg.primaryColor }}
              className="font-black text-[24px] leading-[1.05] tracking-tight uppercase font-['Barlow_Condensed',sans-serif]"
            >
            ...
            <div 
              style={{ backgroundColor: cfg.accentColor }}
              className="w-12 h-[2.5px] rounded-full my-2"
            />

            {/* Key-Value Vitals with Aligned Colons */}
            <div className="space-y-1.5 font-['Barlow_Condensed',sans-serif] text-[14px] leading-tight">
              {cfg.showClass && (
                <div className="grid grid-cols-[68px_12px_1fr] items-baseline">
                  <span style={{ color: cfg.primaryColor }} className="font-bold">{cfg.classLabel}</span>
                  <span style={{ color: cfg.primaryColor }} className="font-bold">:</span>
              ...
              {cfg.showAdmissionNo && (
                <div className="grid grid-cols-[68px_12px_1fr] items-baseline">
                  <span style={{ color: cfg.primaryColor }} className="font-bold">{cfg.admissionNoLabel}</span>
                  <span style={{ color: cfg.primaryColor }} className="font-bold">:</span>
              ...
              {cfg.showDob && (
                <div className="grid grid-cols-[68px_12px_1fr] items-baseline">
                  <span style={{ color: cfg.primaryColor }} className="font-bold">{cfg.dobLabel}</span>
                  <span style={{ color: cfg.primaryColor }} className="font-bold">:</span>
              ...
              {cfg.showBloodGroup && (
                <div className="grid grid-cols-[68px_12px_1fr] items-baseline">
                  <span style={{ color: cfg.primaryColor }} className="font-bold">{cfg.bloodGroupLabel}</span>
                  <span style={{ color: cfg.primaryColor }} className="font-bold">:</span>
                  <span className="font-semibold text-rose-700 font-mono">{bloodGroup}</span>
                </div>
              )}
              {cfg.showRollNo && (
                <div className="grid grid-cols-[68px_12px_1fr] items-baseline">
                  <span style={{ color: cfg.primaryColor }} className="font-bold">{cfg.rollNoLabel}</span>
                  <span style={{ color: cfg.primaryColor }} className="font-bold">:</span>
                  <span className="font-semibold text-slate-800 font-mono">{rollNo}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {/* 4. Dedicated Horizontal Barcode / QR Section */}
        {cfg.barcodeType === 'barcode' && (
        ...
        {cfg.barcodeType === 'qr' && (
          <div className="w-full px-2 pt-1 pb-0 flex flex-col items-center justify-center">
            <StudentQRCode
              payload={`STUDENT:${admissionNo}:${attendanceBarcodeId}`}
              size={54}
            />
            <span className="text-[10px] font-bold text-slate-900 tracking-[0.2em] uppercase text-center mt-0.5 font-['Barlow_Condensed',sans-serif]">
              {cfg.barcodeLabel}
            </span>
          </div>
        )}

        {cfg.barcodeType === 'both' && (
          <div className="w-full px-3 pt-1 pb-0 flex items-center justify-between gap-2">
            <div className="flex-1 flex flex-col items-center">
              <AttendanceBarcode
                ...
                showText={false}
              />
              <span className="text-[9px] font-bold text-slate-900 tracking-[0.2em] uppercase text-center mt-0.5 font-['Barlow_Condensed',sans-serif]">
                BARCODE
              </span>
            </div>
            <div className="flex flex-col items-center shrink-0">
              <StudentQRCode
                ...
                size={40}
              />
              <span className="text-[8.5px] font-bold text-slate-900 tracking-[0.15em] uppercase text-center mt-0.5 font-['Barlow_Condensed',sans-serif]">
                QR SCAN
              </span>
            </div>
          </div>
      ...
      >
        ...
        {cfg.showMotto && (
          <>
            ...
            <span 
              style={{ color: cfg.goldTextColor }}
              className="font-serif font-bold text-[17px] tracking-wide block font-['Noto_Serif_Devanagari',serif] leading-tight"
            >
              {cfg.sanskritMotto}
            </span>
            ...
            <div className="flex items-center justify-center gap-2 mt-1">
              <div style={{ backgroundColor: cfg.accentColor }} className="w-9 h-[1px]"></div>
