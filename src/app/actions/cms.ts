...
// Using a nested JSONB structure as requested.
const MOCK_CMS_DB: Record<string, Record<string, any>> = {
  global: {
    brand: {
...
export async function getPageContent(slug: string) {
  // We use unstable_cache to simulate Next.js caching. 
  // In a real app, this would wrap a database call.
  // We specify the tags here to allow revalidation.
  const fetchCachedData = unstable_cache(
    async () => {
      // Simulate network latency
      await new Promise((resolve) => setTimeout(resolve, 100));
      return MOCK_CMS_DB[slug] || {};
    },
    [`cms-${slug}`],
    {
      tags: [`cms-${slug}`],
...
export async function updateContentBlock(slug: string, section: string, key: string, newValue: string) {
  ...
  if (!MOCK_CMS_DB[slug][section]) {
    MOCK_CMS_DB[slug][section] = {};
  }
