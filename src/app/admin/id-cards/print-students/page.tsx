...
import { 
  Printer, ArrowLeft, User, QrCode, Building2, Phone, Sparkles 
} from "lucide-react";
import { useCampusContext } from "@/components/providers/CampusProvider";
import { getStudentsForIdCardGeneration } from "@/app/actions/id-cards";
...
function StudentIdCardBatchPrintContent() {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-6 font-sans">
      ...
      <div className="print:hidden flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 sm:p-8 rounded-3xl border border-stone-200 shadow-sm">
        <div>
          ...
          <p className="text-stone-500 text-xs sm:text-sm mt-1">
            Printing {students.length} cards for {classFilter} (Academic Session 2026-2027).
          </p>
        </div>
      ...
      <div id="printable-student-cards" className="grid grid-cols-1 sm:grid-cols-2 gap-6 print:grid-cols-2 print:gap-4 print:p-0">
        {students.map((student, idx) => {
          const cleanAdm = student.admission_no || `CB10${(idx + 1).toString().padStart(2, '0')}`;

          return (
            <div key={student.id} className="space-y-4 print:break-inside-avoid print:page-break-inside-avoid">
              ...
              {(layoutMode === "front" || layoutMode === "both") && (
                <div className="bg-white rounded-2xl border-2 border-stone-800 shadow-md p-4 flex flex-col justify-between h-[225px] relative overflow-hidden print:shadow-none">
                  <div className="flex items-center justify-between border-b-2 border-stone-900 pb-2">
                    ...
                    <span className="text-[9px] font-mono font-black bg-stone-100 px-1.5 py-0.5 rounded border border-stone-300">
              ...
              {(layoutMode === "back" || layoutMode === "both") && (
                <div className="bg-white rounded-2xl border-2 border-stone-800 shadow-md p-4 flex flex-col justify-between h-[225px] relative overflow-hidden print:shadow-none">
                  <div className="border-b border-stone-200 pb-1 flex justify-between items-center">
                    <span className="font-black text-[10px] text-stone-900 uppercase">Emergency &amp; Security Protocol</span>
                    <span className="font-mono text-[8px] text-stone-400">ID: {student.card_number}</span>
                  </div>
                  ...
                  <div className="space-y-1 text-[9px] text-stone-600 flex-1 py-1">
                    ...
                    <p><span className="font-bold text-stone-800">School Phone:</span> +91 98100 81008, 011-28753300</p>
                    <p className="text-[8px] text-stone-500 pt-1">
                      1. This card must be worn by the student at all times during school hours and transport.
                    </p>
                    <p className="text-[8px] text-stone-500">
                      2. If lost, report immediately to the school office for instant token invalidation.
                    </p>
                  </div>
                  ...
                  <div className="border-t border-stone-300 pt-2 flex justify-between items-end text-[8px]">
                    <div>
                      ...
                      <span className="text-[7px] text-stone-400">Burari, Delhi - 110084</span>
                    </div>
