    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-6 font-sans">
      ...
      {activeTab === "exit" && (
        <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-stone-100 pb-4">
            <div>
              ...
              <p className="text-xs text-stone-500">
                Statutory compliant offboarding, work reassignment to replacement faculty, and profile archival.
              </p>
            </div>
          ...
          {/* Institutional Audit Compliance Guarantee Card */}
          <div className="p-5 bg-emerald-50 rounded-2xl border border-emerald-200 text-emerald-950 space-y-3">
            <h4 className="font-black text-emerald-900 text-sm flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-emerald-700" /> Institutional Audit Compliance &amp; Data Integrity Standard
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  ...
                  <div className="p-3.5 bg-blue-50/70 rounded-2xl border border-blue-100 text-xs">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="bg-blue-200/80 text-blue-900 px-2 py-0.5 rounded-full text-[10px] font-bold">
                        {((editForm.role || "Teacher / Faculty").split(",").map((r: string) => r.trim()).filter(Boolean)).length} Selected
                      </span>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 max-h-56 overflow-y-auto pr-1">
                      {ERP_ROLES_LIST.map((r) => {
                        const curRoles = (editForm.role || "Teacher / Faculty").split(",").map((x: string) => x.trim()).filter(Boolean);
                        ...
                        return (
                          <div
                            ...
                            onClick={() => {
                              ...
                              setEditForm({ ...editForm, role: updated.join(", ") });
                            }}
