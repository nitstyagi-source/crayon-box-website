...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { title, subject, description, dueDate, teacherName, targetClass, periodNumber = 1, campusId: reqCampusId, academicSession: reqSession } = body;

    const curYear = new Date().getFullYear();
    const academicSession = reqSession || `${curYear}-${(curYear + 1).toString().slice(-2)}`;
    ...
    const res = await client.query(`
      INSERT INTO public.digital_diary_entries (
        ...
        $1, $9, CURRENT_DATE, TO_CHAR(CURRENT_DATE, 'Day'), $2, 'Period 1', $3, 'A', $4,
        $5, $6, $7, $8, NOW(), NOW()
      )
    ...
    `, [
      ...
      title,
      description || '',
      dueDate ? new Date(dueDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
      academicSession
    ]);
